...
unsigned long get_page_attribute(struct Page *page)
{
	if (page == NULL)
	{
		LOGE("get_page_attribute() ERROR: page == NULL");
		return 0;
	}
	else
...
unsigned long set_page_attribute(struct Page *page, unsigned long flags)
{
	if (page == NULL)
	{
		LOGE("set_page_attribute() ERROR: page == NULL");
		return 0;
	}
	else
	{
...
void do_init_memory()
{
	...
	LOGI("Display Physics Address MAP,Type(1:RAM,2:ROM or Reserved,3:ACPI Reclaim Memory,4:ACPI NVS Memory,Others:Undefine)");
	p = (struct E820[2]){
		[0] = {
			.address = &__text_start,
			.length = (&__heap_end - &__text_start),
			.type = 1,
		},
		[1] = {
			.address = 0,
	...
	mms.start_code = (unsigned long)&__text_start;
	mms.end_code = (unsigned long)&__text_end;
	mms.end_data = (unsigned long)&__data_end;
	mms.end_rodata = (unsigned long)&__rodata_end;
	mms.start_brk = (unsigned long)&__heap_start;

	for (i = 0; i < 32; i++)
	{
		if (p->type == 1)
			LOGD("Address:%#010lx\tLength:%#010lx\tType:%#010x", p->address, p->length, p->type);
		if (p->type == 1)
			TotalMem += p->length;
	...
	LOGD("OS Can Used Total RAM:%#010lx", TotalMem);
	...
	LOGD("OS Can Used Total 2M PAGEs:%#010lx=%d", TotalMem, TotalMem);
	...
	LOGD("bits_map:%#010lx,bits_size:%#010lx,bits_length:%#010lx", mms.bits_map, mms.bits_size, mms.bits_length);
	LOGD("pages_struct:%#010lx,pages_size:%#010lx,pages_length:%#010lx", mms.pages_struct, mms.pages_size, mms.pages_length);
	LOGD("zones_struct:%#010lx,zones_size:%#010lx,zones_length:%#010lx", mms.zones_struct, mms.zones_size, mms.zones_length);
	...
	for (i = 0; i < mms.zones_size; i++)
	{
		...
		LOGD("zone_start_address:%#010lx,zone_end_address:%#010lx,zone_length:%#010lx,pages_group:%#010lx,pages_length:%#010lx", z->zone_start_address, z->zone_end_address, z->zone_length, z->pages_group, z->pages_length);

		//if (z->zone_start_address >= 0x100000000 && !ZONE_UNMAPED_INDEX)
		//	ZONE_UNMAPED_INDEX = i;
	}

	LOGD("ZONE_DMA_INDEX:%d\tZONE_NORMAL_INDEX:%d\tZONE_UNMAPED_INDEX:%d", ZONE_DMA_INDEX, ZONE_NORMAL_INDEX, ZONE_UNMAPED_INDEX);
	...
	LOGD("start_code:%#010lx,end_code:%#010lx,end_data:%#010lx,start_brk:%#010lx,end_of_struct:%#010lx", mms.start_code, mms.end_code, mms.end_data, mms.start_brk, mms.end_of_struct);
	...
	LOGD("1.mms.bits_map:%#010lx\tzone_struct->page_using_count:%d\tzone_struct->page_free_count:%d", *mms.bits_map, mms.zones_struct->page_using_count, mms.zones_struct->page_free_count);
...
struct Page *alloc_pages(int zone_select, int number, unsigned long page_flags)
{
	...
	{
		LOGE("alloc_pages() ERROR: number is invalid");
		goto ret_null;
	}
	...
	default:
		LOGE("alloc_pages() ERROR: zone_select index is invalid");
		goto ret_null;
		break;
	}
	...
	LOGE("alloc_pages() ERROR: no page can alloc");
ret_null:
	assert(0);
	return NULL;
...
void *_alloc_pages(int zone_select, unsigned long size, unsigned long page_flags)
{
...
void free_pages(struct Page *page, int number)
{
	if (page == NULL)
	{
		LOGE("free_pages() ERROR: page is invalid.");
		return;
	}

	if (number >= BITS_PER_LONG || number <= 0)
	{
		LOGE("free_pages() ERROR: number is invalid. number = %d", number);
		return;
	}
...
void *kmalloc(unsigned long size, unsigned long gfp_flages)
{
	...
	if (size > 1048576)
	{
		//LOGE("kmalloc() ERROR: kmalloc size too long:%08d", size);
		//goto ret_null;
		void *addr = _alloc_pages(ZONE_NORMAL, size, 0);
		if (addr == NULL)
		{
			LOGE("kmalloc() ERROR: _alloc_pages :%08d", size);
		}
		return addr;
	}
	...
	if (kmalloc_cache_size[i].total_free != 0)
	{
		while (1)
		{
			if (slab->free_count == 0)
				slab = list_next_entry(slab, list);
			else
				break;
			if (slab == kmalloc_cache_size[i].cache_pool)
			{
				LOGF("kmalloc() ERROR: kmalloc_cache_size[i].total_free != 0");
				goto ret_null;
			}
		}
	}
	else
	{
		...
		if (slab == NULL)
		{
			LOGF("kmalloc()->kmalloc_create()=>slab == NULL");
			goto ret_null;
		}

		kmalloc_cache_size[i].total_free += slab->color_count;

		LOGF("kmalloc()->kmalloc_create()<=size:%#010x", kmalloc_cache_size[i].size); ///////
	...
	LOGF("kmalloc() ERROR: no memory can alloc");
ret_null:
	assert(0);
	return NULL;
}

void *kzalloc(size_t size, unsigned long gfp_flages)
{
	void *m = NULL;

	if ((m = kmalloc(size, gfp_flages)))
		memset(m, 0, size);
	return m;
}
...
struct Slab *kmalloc_create(unsigned long size)
{
	...
	if (page == NULL)
	{
		LOGE("kmalloc_create()->alloc_pages()=>page == NULL");
		return NULL;
	}
	...
	default:
		...
		LOGE("kmalloc_create() ERROR: wrong size:%08d", size);
		free_pages(page, 1);
...
unsigned long kfree(void *address)
{
	...
	LOGE("kfree() ERROR: can`t free memory");
	return 0;
}
...
struct Slab_cache *slab_create(unsigned long size, void *(*constructor)(void *Vaddress, unsigned long arg), void *(*destructor)(void *Vaddress, unsigned long arg), unsigned long arg)
{
	...
	if (slab_cache == NULL)
	{
		LOGE("slab_create()->kmalloc()=>slab_cache == NULL");
		return NULL;
	}
	...
	{
		LOGE("slab_create()->kmalloc()=>slab_cache->cache_pool == NULL");
		kfree(slab_cache);
		return NULL;
	}
	...
	init_list_head(&slab_cache->cache_pool->list);

	slab_cache->cache_pool->page = alloc_pages(ZONE_NORMAL, 1, 0);

	if (slab_cache->cache_pool->page == NULL)
	{
		LOGE("slab_create()->alloc_pages()=>slab_cache->cache_pool->page == NULL");
	...
	{
		LOGE("slab_create()->kmalloc()=>slab_cache->cache_pool->color_map == NULL");
...
unsigned long slab_destroy(struct Slab_cache *slab_cache)
{
	...
	{
		LOGE("slab_cache->total_using != 0");
		return 0;
	}

	while (!list_empty(&slab->list))
	{
		list_del(&slab->list);
		kfree(slab->color_map);
...
void *slab_malloc(struct Slab_cache *slab_cache, unsigned long arg)
{
	...
	if (slab_cache->total_free == 0)
	{
		...
		if (slab == NULL)
		{
			LOGE("slab_malloc()->kmalloc()=>tmp_slab == NULL");
			return NULL;
		}
		...
		if (slab->page == NULL)
		{
			LOGE("slab_malloc()->alloc_pages()=>tmp_slab->page == NULL");
			kfree(slab);
			return NULL;
		}
		...
		{
			LOGE("slab_malloc()->kmalloc()=>tmp_slab->color_map == NULL");
			free_pages(slab->page, 1);
			kfree(slab);
			return NULL;
		}
		...
		list_add(&slab->list, &slab_cache->cache_pool->list);
	...
	else
	{
		while (1)
		{
			...
			{
				LOGF("slab_malloc() ERROR: slab_cache->total_free != 0");
				return NULL;
			}
		}
	}
	...
	LOGE("slab_malloc() ERROR: can`t alloc");
...
unsigned long slab_free(struct Slab_cache *slab_cache, void *address, unsigned long arg)
{
	...
	LOGE("slab_free() ERROR: address not in slab");
...
unsigned long do_init_slab()
{
	...
	for (i = 0; i < ARRAY_SIZE(kmalloc_cache_size); i++)
	{
		...
		init_list_head(&kmalloc_cache_size[i].cache_pool->list);
	...
	LOGD("2.mms.bits_map:%#010lx\tzone_struct->page_using_count:%d\tzone_struct->page_free_count:%d", *mms.bits_map, mms.zones_struct->page_using_count, mms.zones_struct->page_free_count);
	...
	LOGD("3.mms.bits_map:%#010lx\tzone_struct->page_using_count:%d\tzone_struct->page_free_count:%d", *mms.bits_map, mms.zones_struct->page_using_count, mms.zones_struct->page_free_count);
