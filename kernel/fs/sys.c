...
{
	LOGE("no_system_call is calling");
	return -ENOSYS;
}

unsigned long sys_putstring(char *string)
{
	LOGD(ORANGE, WHITE, "%s", string);
	return 0;
}
...
unsigned long sys_reboot(unsigned long cmd, void *arg)
{
	...
	{
	case SYSTEM_REBOOT:
		LOGE("sys_reboot cmd SYSTEM_REBOOT!");
		break;
	case SYSTEM_POWEROFF:
		LOGE("sys_reboot cmd SYSTEM_POWEROFF");
		break;

	default:
		LOGE("sys_reboot cmd ERROR!");
		break;
	}
	return 0;
}
