...
static int timer_function(struct timer_t * timer, void * data)
{
	//timer_cancel(timer);
	lv_tick_inc(1);
...
void init_tick()
{
	timer_init(&timer, timer_function, "hello");
...
static void ex_disp_flush(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t * color_p)
{
    ...
    for(y = y1; y <= y2; y++) {
        for(x = x1; x <= x2; x++) {
            ...
            /* put_px(x, y, *color_p)*/
			((u32_t *)render->pixels)[y * width + x] = color_p->full;
			color_p++;
        }
    }
...
static bool ex_tp_read(lv_indev_data_t * data)
{
	...
	//printf("[%4d %4d]\r", data->point.x, data->point.y);
    return true;
}
