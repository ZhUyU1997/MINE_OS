...
static struct vt100 *term;
static void _puts(char *str){
	console->write(console, str, strlen(str));
}
...
static unsigned long test(unsigned long arg)
{
	test_cursor();
	test_scroll();
	test_edit();
...
void test_vt100(struct console_t * _console, struct vt100 *_term){
	console = _console;
	term = _term;
	kernel_thread(test, 10, CLONE_FS | CLONE_SIGHAND);
}
...
void test_cursor(){
	...
	// draw a line of "*"
	for(int c = 0; c < term->width; c++){
		_puts("*"); 
	}
	// draw left and right border
	for(int c = 0; c < term->height; c++){
		sprintf(buf, "\e[%d;1H*\e[%d;%dH*", c + 1, c + 1, term->width);
		_puts(buf);
	}
	// draw bottom line
	sprintf(buf, "\e[%d;1H", term->height);
	_puts(buf); 
	for(int c = 0; c < term->width; c++){
		_putc('*');
	}
	...
	_puts("\e[2;2H");
	// draw a line of "*"
	for(int c = 0; c < term->width - 2; c++){
		_putc('+'); 
	}
	// draw left and right border
	for(int c = 1; c < term->height - 1; c++){
		sprintf(buf, "\e[%d;2H+\e[%d;%dH+", c + 1, c + 1, term->width - 1);
		_puts(buf);
	}
	// draw bottom line
	sprintf(buf, "\e[%d;2H", term->height - 1);
	_puts(buf); 
	for(int c = 0; c < term->width - 2; c++){
		_putc('+');
	}
	...
	for(int c = 1; c < term->height; c++){
		sprintf(buf, "\e[%d;1H*+\e[%d;%dH+*", c + 1, c + 1, term->width - 1);
		_puts(buf);
	}
...
void test_scroll(){
	...
	_puts("\e[1;1H"); for(int x = 0; x < term->width; x++) _putc('#'),mdelay(1); 
	_puts("\e[3;1H"); for(int x = 0; x < term->width; x++) _putc('#'),mdelay(1); 
	_puts("\e[30;1H"); for(int x = 0; x < term->width; x++) _putc('#'),mdelay(1); 
	_puts("\e[34;1H"); for(int x = 0; x < term->width; x++) _putc('#'),mdelay(1);
	...
	_puts("\e[99;1H"); for(int x = 0; x < term->width; x++) _putc('!'),mdelay(1);
	for(int y = 0; y < term->height; y++){
		//sprintf(buf, "\e[%d;1H!\e[%d;%dH!", y+1, y+1, term->width);
...
void test_edit(){
	...
	for(int c = 23; c < term->height; c+=2){
