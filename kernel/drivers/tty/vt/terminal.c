...
uint16_t PIXEL_X(uint16_t a) {
	return a;
}
uint16_t PIXEL_Y(uint16_t a) {
	if (a >= term.top && a < term.bottom) {
		if (a >= term.scroll_start)
...
void terminal_init(void) {
	term.screen_width = TFTWIDTH;
	term.screen_height = TFTHEIGHT;
	term.char_height = 8;
	term.char_width = 6;
	...
	term.scroll_start = 0;
	term.top = 0;
	term.bottom = TFTHEIGHT;
}

void setScrollStart(uint16_t start) {
	//uint16_t temp = (term.bottom - term.top + start - term.scroll_start) % (term.bottom - term.top);
	//printf("num : %d\n",PIXEL_Y(start) - term.top);
	ScrollLine(term.top, term.bottom, PIXEL_Y(start) - term.top);
...
void drawChar(uint16_t x, uint16_t y, uint8_t ch) {
	...
	for (int b = 0; b < t->char_height; b++) {
		...
		for (int j = 0; j < t->char_width; j++) {	
			...
			if (j != t->char_width - 1 && _buf[j] & _BV(b))
				pix = t->front_color;
			PutPixel(x + j, y + b, pix);
...
void setRotation(uint8_t m) {
	...
	switch (rotation) {
		case 0:
			t->screen_width  = TFTWIDTH;
			t->screen_height = TFTHEIGHT;
			break;
		case 1:
			t->screen_width  = TFTHEIGHT;
			t->screen_height = TFTWIDTH;
			break;
		case 2:
			t->screen_width  = TFTWIDTH;
			t->screen_height = TFTHEIGHT;
