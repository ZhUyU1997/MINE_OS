...
inline uint16_t VT100_CURSOR_Y(struct vt100 *t) {
	...
	if (t->cursor_y < t->scroll_start_row || t->cursor_y >= t->scroll_end_row) {
		...
		row = (row - t->scroll_start_row) % scroll_height + t->scroll_start_row;
		//TODO:
			...
			if (isdigit(arg)) { // start of an argument
				...
				switch (arg) {
					...
					case 'H': { // move cursor to position (default 0;0)
						...
						if (term->cursor_x >= VT100_WIDTH) term->cursor_x = VT100_WIDTH - 1;
						if (term->cursor_y >= VT100_HEIGHT) term->cursor_y = VT100_HEIGHT - 1;
						term->state = _st_idle;
						break;
					}
					...
					case 'r': // Set scroll region (top and bottom margins)
						...
						if (term->narg == 2 && term->args[0] < term->args[1]) {
							...
							term->scroll_end_row = term->args[1];
							//TODO:错误处理
							//if(term->scroll_start_row > VT100_HEIGHT) term->scroll_end_row = VT100_CHAR_HEIGHT;
							if(term->scroll_end_row > VT100_HEIGHT) term->scroll_end_row = VT100_CHAR_HEIGHT;
